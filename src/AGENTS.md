...
- 用户可见的 `Canvas` 一律称为“编组方案”；内部 Rust、IPC、数据库标识继续使用 Canvas，禁止进行无价值的破坏性重命名。
- 项目层级固定为“项目 → 文件夹 → 编组方案”；项目/文件夹只负责组织，编组方案才是图、出口、运行、文档和记录的隔离边界。
...
- 删除、停用、改名等确认统一使用应用中央模态层，禁止 `window.confirm` 和 `window.prompt`。
- 主控台实例卡片必须按名称和稳定 ID 排序，不能因运行状态变化重排。
