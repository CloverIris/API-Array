...
use apiarray_core::{
    ...
    capability::Evidence,
    inspection::{
        InspectionDimension, InspectionFinding, InspectionOverall, InspectionReport,
        InspectionStatus,
    },
...
impl ProviderProbeRunner {
    ...
    pub async fn run(
        ...
        let billable = instance
            ...
            .filter(|probe| {
                matches!(
                    probe.side_effect,
                    apiarray_core::provider::ProbeSideEffect::Billable
                )
            })
            .count();
        findings.push(InspectionFinding {
            dimension: InspectionDimension::ContextWindow,
            status: if billable > 0 && !allow_billable {
                InspectionStatus::AuthorizationRequired
            } else {
                InspectionStatus::NotRun
            },
            evidence: Evidence::ProviderDeclared,
            latency_ms: None,
            safe_summary: Some(
                if billable > 0 && !allow_billable {
                    "存在需授权的计费探测，当前未执行"
                } else {
                    "当前 Provider 未声明无副作用上下文探测"
                }
                .to_owned(),
            ),
            declared_value: None,
        ...
        let overall = if findings
            .iter()
            .any(|finding| finding.status == InspectionStatus::Failed)
        {
    ...
    async fn authentication(
        ...
        match secrets.resolve(reference) {
            Ok(_) => passed(
                InspectionDimension::Authentication,
                started.elapsed().as_millis() as u64,
                "本机 Secret 已绑定",
            ),
    ...
    async fn models(
        ...
        let path = match model_discovery_path(instance) {
            Some(path) => path,
            None if matches!(
                instance.manifest.discovery.models.strategy,
                DiscoveryKind::Static
            ) =>
            {
                return passed(InspectionDimension::Models, 0, "Provider 使用静态模型声明");
            }
...
fn model_discovery_path(instance: &ProviderInstance) -> Option<&str> {
    match instance.manifest.discovery.models.strategy {
        ...
        DiscoveryKind::Adapter if instance.manifest.adapter.id == "google-gemini" => {
            Some("/v1beta/models")
        }
...
fn discovery_plan(instance: &ProviderInstance, path: &str) -> Result<TransportPlan, RuntimeError> {
    let base = instance
        .endpoint_override
        .as_deref()
        .unwrap_or(&instance.manifest.endpoint.default_base_url)
        .trim_end_matches('/');
    let url = if path == "/" {
        base.to_owned()
    } else {
        format!("{base}/{path}", path = path.trim_start_matches('/'))
    };
    let mut headers = instance
        .manifest
        .headers
        .iter()
    ...
    for field in &instance.manifest.authentication.fields {
        ...
        let Some(reference) = instance.secret_refs.get(&field.id).cloned() else {
            continue;
        };
        let value = HeaderValue::SecretRef {
            reference,
            prefix: field.prefix.clone().unwrap_or_default(),
        };
        match instance.manifest.authentication.kind {
            AuthenticationType::HeaderSecret => {
            ...
            AuthenticationType::QuerySecret => {
                if let Some(name) = &field.query {
                    query.push(QueryPlan {
                        name: name.clone(),
                        value,
                    });
                }
    ...
    Ok(TransportPlan {
        adapter: apiarray_core::adapter::AdapterKind::parse(&instance.manifest.adapter.id)
            .map_err(RuntimeError::from)?,
...
fn passed(dimension: InspectionDimension, latency_ms: u64, summary: &str) -> InspectionFinding {
    InspectionFinding {
        dimension,
        status: InspectionStatus::Passed,
        evidence: Evidence::Verified,
        latency_ms: Some(latency_ms),
        safe_summary: Some(summary.to_owned()),
        declared_value: None,
        verified_value: None,
    }
}
fn failed(dimension: InspectionDimension, summary: String) -> InspectionFinding {
    InspectionFinding {
        dimension,
        status: InspectionStatus::Failed,
        evidence: Evidence::VerificationFailed,
        latency_ms: None,
...
fn unsupported(dimension: InspectionDimension, summary: &str) -> InspectionFinding {
    InspectionFinding {
        dimension,
...
impl InspectionRepository {
    pub fn new(root: impl AsRef<Path>) -> Self {
        Self {
            repository: crate::persistence::WorkspaceRepository::new(root.as_ref()),
        }
    }
    #[must_use]
    pub fn from_repository(repository: crate::persistence::WorkspaceRepository) -> Self {
        Self { repository }
    }
...
mod tests {
    ...
    #[test]
    fn report_repository_round_trips_without_using_provider_id_as_a_path()
    -> Result<(), RuntimeError> {
