...
import { ApiKeys, ArrowRotateCcw, Branch, CheckCircle, Code, Copy, Document, DotsHorizontal, Grid, History, Pause, Play, Plugin, Search, Settings as SettingsIcon, Warning } from "@openai/apps-sdk-ui/components/Icon";
...
type Props = { workspaceName: string; controlCenter: ControlCenterSnapshot; wallet: WalletCard[]; projectTree: ProjectTree; onControlCenter: (snapshot: ControlCenterSnapshot) => void; onRefresh: () => void; onNavigate: (page: AppPage) => void; onOpenCanvas: (projectId: string, canvasId: string, tab?: "overview" | "workflow" | "publisher" | "docs") => void };
const statusMeta: Record<ManagedInstanceStatus, { label: string; color: "success" | "secondary" | "warning" | "danger" | "info" }> = { running: { label: "运行中", color: "success" }, stopped: { label: "已停止", color: "secondary" }, starting: { label: "启动中", color: "info" }, stopping: { label: "停止中", color: "warning" }, blocked: { label: "需要处理", color: "warning" }, failed: { label: "运行异常", color: "danger" }, unpublished: { label: "尚未发布", color: "secondary" } };
...
export function HomeDashboard({ workspaceName, controlCenter, wallet, projectTree, onControlCenter, onRefresh, onNavigate, onOpenCanvas }: Props) {
  const [view, setView] = useState<"overview" | "instances">("overview"); const [query, setQuery] = useState(""); const [kind, setKind] = useState<"all" | ManagedInstanceKind>("all"); const [status, setStatus] = useState<"all" | ManagedInstanceStatus>("all"); const [busy, setBusy] = useState<string | null>(null); const [notice, setNotice] = useState<string | null>(null); const [error, setError] = useState<string | null>(null);
  const instances = controlCenter.instances; const plans = Object.values(projectTree.projects ?? {}).flatMap((project) => Object.values(project.canvases ?? {}));
  const visible = useMemo(() => instances.filter((item) => (`${item.name} ${item.ownership} ${item.assetNames.join(" ")} ${item.publicModels.join(" ")}`).toLowerCase().includes(query.trim().toLowerCase()) && (kind === "all" || item.kind === kind) && (status === "all" || item.status === status)), [instances, query, kind, status]);
  const recent = [...instances].sort((a, b) => a.name.localeCompare(b.name, "zh-CN") || a.id.localeCompare(b.id)).slice(0, 6); const attention = instances.filter((item) => ["blocked", "failed", "unpublished"].includes(item.status) || item.hasUnappliedChanges);
  const update = (result: InstanceBatchResult) => { onControlCenter(result.snapshot); onRefresh(); setNotice(`${result.succeeded} 项已处理，${result.failed} 项需要修复。`); };
  const toggle = async (item: ManagedInstance) => { setBusy(item.id); try { update(await (item.status === "running" ? stopManagedInstance(item.id) : startManagedInstance(item.id))); } catch (reason) { setError(readError(reason)); } finally { setBusy(null); } };
  const test = async (item: ManagedInstance) => { setBusy(`test:${item.id}`); try { const result = await testManagedInstance(item.id); setNotice(`${item.name}：${result.safeSummary}，${result.latencyMs} ms。`); } catch (reason) { setError(readError(reason)); } finally { setBusy(null); } };
  const openDetails = (item: ManagedInstance) => { if (item.kind === "direct_endpoint") { if (item.directEndpointId) sessionStorage.setItem("apiarray.direct.endpoint", item.directEndpointId); onNavigate(item.repairTarget === "wallet" ? "wallet" : "direct"); } else if (item.projectId && item.canvasId) onOpenCanvas(item.projectId, item.canvasId, item.repairTarget === "canvas:publisher" ? "publisher" : item.repairTarget === "canvas:workflow" ? "workflow" : "overview"); };
  const docs = (item: ManagedInstance) => { if (item.kind === "direct_endpoint") { if (item.directEndpointId) sessionStorage.setItem("apiarray.direct.docs", item.directEndpointId); onNavigate("direct"); } else if (item.projectId && item.canvasId) onOpenCanvas(item.projectId, item.canvasId, "docs"); };
  const startAll = async () => { setBusy("all"); try { update(await startAllInstances()); } catch (reason) { setError(readError(reason)); } finally { setBusy(null); } };
  const stopAll = async () => { if (!await askAppDialog({ title: "停止全部业务入口？", description: "统一网关仍会运行，配置、Secret 和审计记录不会删除。", confirmLabel: "停止全部", danger: true })) return; setBusy("all"); try { update(await stopAllInstances()); } catch (reason) { setError(readError(reason)); } finally { setBusy(null); } };
