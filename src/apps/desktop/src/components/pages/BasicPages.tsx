...
export function SettingsPage({ onSnapshot, uiState, onUiState }: { onSnapshot: (snapshot: DesktopSnapshot) => void; uiState: WorkspaceUiState; onUiState: (state: WorkspaceUiState) => void }) {
  ...
  return <div className="settings-view"><section className="appearance-card"><div><p className="eyebrow">Appearance</p><h2>外观与工作区视图</h2><p>默认跟随 Windows，也可以为 API ARRAY 单独设置。</p></div><label>主题<SegmentedControl value={uiState.themePreference} onChange={(themePreference) => onUiState({ ...uiState, themePreference })} aria-label="主题偏好" block><SegmentedControl.Option value="system">系统</SegmentedControl.Option><SegmentedControl.Option value="light">亮色</SegmentedControl.Option><SegmentedControl.Option value="dark">暗色</SegmentedControl.Option></SegmentedControl></label><label>默认视图<SegmentedControl value={uiState.viewMode} onChange={(viewMode) => onUiState({ ...uiState, viewMode })} aria-label="默认工作区视图" block><SegmentedControl.Option value="simple">简洁</SegmentedControl.Option><SegmentedControl.Option value="professional">专业</SegmentedControl.Option></SegmentedControl></label></section><section className="setting-card"><div><div className="setting-title"><SettingsIcon className="size-5" /><h2>开机启动</h2></div><p>仅保存为此设备的桌面偏好，不写入业务工作区。</p></div><Switch checked={enabled ?? false} disabled={busy || enabled === null} label={enabled ? "已开启" : "已关闭"} onCheckedChange={(next) => void toggle(next)} /></section><section className="workspace-transfer"><div className="section-heading"><div><p className="eyebrow">Workspace Transfer</p><h2>导入与导出</h2></div><Badge color="info" variant="soft">Secret 仅保留引用</Badge></div>{message ? <p className="error-message" role="alert">{message}</p> : null}<Textarea value={workspaceJson} onChange={(event) => setWorkspaceJson(event.target.value)} rows={12} placeholder="导出工作区后显示 JSON；也可以在此粘贴另一台设备导出的工作区。" /><div className="form-actions"><Button color="secondary" variant="soft" loading={busy} onClick={() => void exportCurrent()}><FileDocument />导出并复制</Button><Button color="primary" loading={busy} disabled={!workspaceJson.trim()} onClick={() => void importCurrent()}><CheckCircle />导入工作区</Button></div></section></div>;
}
