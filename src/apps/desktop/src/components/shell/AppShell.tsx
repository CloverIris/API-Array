...
type Props = {
  ...
  onRefresh: () => void;
  onUiState: (next: WorkspaceUiState) => void;
};
...
export function AppShell({
  ...
  const contentClass = route.kind === "canvas" || route.page === "wallet"
    ? "canvas-page"
    : route.page === "home"
      ? "dashboard-page"
      : "reading-page";
  ...
  return (
    <main className="desktop-shell">
      <UnifiedTopBar
        ...
        projectTree={projectTree}
        theme={uiState.themePreference}
        onTheme={(themePreference) => onUiState({ ...uiState, themePreference })}
        onNavigate={onNavigate}
        onOpenCanvas={onOpenCanvas}
        onRefresh={onRefresh}
        onOpenInspector={() => patchShell({ rightInspectorOpen: true })}
      />
      <div className="three-columns" style={{ gridTemplateColumns: columns }}>
        <LeftSidebar
          page={global?.id ?? null}
          tree={projectTree}
          publishers={control.supervisor.publishers ?? []}
          uiState={uiState}
          collapsed={shell.leftSidebarCollapsed}
          onNavigate={onNavigate}
          onOpenCanvas={onOpenCanvas}
          onTree={onTree}
          onUiState={onUiState}
          onToggle={() => patchShell({ leftSidebarCollapsed: !shell.leftSidebarCollapsed })}
        />
        <section className={`content-panel ${route.kind === "canvas" ? "page-canvas" : `page-${route.page}`}`}>
      ...
      <RuntimeStatusBar control={control} runningCount={controlCenter?.runningCount ?? 0} />
    </main>
  );
}
