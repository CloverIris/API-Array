...
- 新用户默认一键创建工作区，数据库放在程序目录的 `workspace/` 中；OOBE 不强迫用户先理解数据库路径。
- “自定义存储位置”和“打开已有工作区”作为次级入口；创建后仍可在设置中复制、验证并迁移。
- 统一网关默认监听 `127.0.0.1:7480`。设置页允许选择 `127.0.0.1` 或 `::1` 并配置 `1024–65535` 端口，仍禁止公网监听。
- 地址或端口变更必须事务保存并重启 LocalGateway；启动失败时恢复旧配置和旧监听器。
- 删除、停用、重命名和其他许可操作统一使用应用中央模态层，不得调用浏览器原生 `confirm` 或 `prompt`。
- 主页实例卡片使用名称与稳定 ID 排序，运行状态变化不得导致卡片互换位置。
